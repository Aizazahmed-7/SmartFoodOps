...
| POST `/v1/orders` | auth (customer or restaurant_admin) | **required** (§4 in full: replay w/ `Idempotent-Replay: true`, reuse 422, in-progress 409; deterministic refusals release the key) | PLACEMENT | order | 202 `{order_id, status}`; **the saga writes it** (ADR-0023): the route prices + reserves the key, then update-with-start runs `create_order`, whose one tx = order + line/address/pricing snapshots + OrderPlaced outbox + idempotency completion. `order_id` is derived from the key. Await > `placement_await_seconds` still answers 202 (row may 404 briefly); Temporal unreachable = 503 with the key **held**, so the retry re-derives the same order |
