...
All POSTs that create or mutate orders/money require `Idempotency-Key` (client-generated UUID, ≤64 chars). GET/DELETE never take the header. Two implementations of the same promise: **payment** runs the full `libs/smartfood-idempotency` table protocol below (stored 402s must replay verbatim); **order placement** retired its table (ADR-0024) — the key derives the order id, and the orders row itself is the record.
...
| GET `/v1/menus/{rid}` | public_read | – | READ | catalog | the effective menu, cache-aside with a 5-min TTL (ADR-0027); no version field — it left the wire with the version columns (ADR-0037) |
| POST `/v1/orders` | auth (customer or restaurant_admin) | **required** — the derivation seed, not a ledger (ADR-0024): `order_id = uuid5(sub:key)`; row exists + hash matches → 202 replay (current status, `Idempotent-Replay: true`); hash differs → 422 reuse; no 409-in-progress state exists | PLACEMENT | order | 202 `{order_id, status}`; **the saga writes it** (ADR-0023): the route reads the row, prices, then update-with-start runs `create_order` (one tx = order + line/address/pricing snapshots + `request_hash` + OrderPlaced outbox). Await > `placement_await_seconds` still answers 202 (row may 404 briefly); Temporal unreachable = 503 with **nothing written** — the retry re-derives the same order; a refusal during the pending window is outranked by the running workflow's ack |
...
| POST `/v1/quote` | auth (customer or restaurant_admin — owners order dinner too) | – (stateless read) | READ | order | pricing lib in-process; never refuses on drift — the response IS the current price, and its `totals.total_cents` is what placement then consents to via `expected_total_cents` (ADR-0036); 409 `ITEM_UNAVAILABLE`/`RESTAURANT_CLOSED`, 422 selection violations, 503 when catalog is down |
