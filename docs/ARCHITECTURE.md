...
| Notification | Decide + log notifications; senders fan out *(As built, W3: consumer-only durable per-recipient inbox, read via API; sender fan-out not built)* | DDB `notification_log` TTL 90d *(As built, W3: `notification_db` — `notifications` + `order_recipients`)* | Consumer: Fargate; senders: **Lambda** | Consumes all topics; Celery/SQS to providers *(As built, W3: consumes `c1.orders.events` + `c1.payments.events` via two `EventConsumer` loops, groups `notification.inbox.orders` / `notification.inbox.payments`; produces nothing; serves `/v1/notifications` reads via edge)* |
...
| Concern | Design |
|---|---|
| Tokens | Identity issues RS256 JWTs, **15-min access** (claims: `sub`, `roles` ⊆ customer/restaurant_admin/rider/system_admin, scoping `restaurant_id`/`rider_id`, `cell`, `jti`) + **30-day opaque rotating refresh tokens** in PG, **one row per login, rotated in place** (ADR-0034). Rotation overwrites the stored hash, so a replayed token is indistinguishable from garbage: reuse detection is deliberately given up, and a stolen token stops working at the holder's next refresh. JWKS endpoint holds two live keys (current+next) for rotation. |
| Verification | **Once, at the edge** (JWKS cached in-process 10 min). Edge strips all inbound `X-Auth-*`, then stamps `X-Auth-Sub`, `X-Auth-Roles`, `X-Auth-Restaurant-Id`, `X-Auth-Rider-Id`. Services consume via shared `smartfood-auth` middleware (`AuthContext` dependency) and never parse JWTs — one hardened verifier beats ten mediocre ones. Headers are trustworthy because domain services are network-unreachable except from edge/gateways/peers. Hardening path (documented, deferred): edge-minted short-lived internal JWT, then mTLS — the middleware hides the swap. |
...
sequenceDiagram
  ...
  Note over T: activity create_order — one PG tx:<br/>order PLACED + lines + request_hash<br/>+ OrderPlaced outbox
...
**Placement is the saga's first step (ADR-0023), and the orders row is its own idempotency record (ADR-0024).** There is no commit→start gap, no sweeper, and no key table: `POST /v1/orders` derives `order_id = uuid5(NS, "{sub}:{idem_key}")`, **reads the orders row first** (exists + `request_hash` matches → 202 replay with the current status; hash differs → 422 reuse), then resolves the address, snapshots the menu and prices in-process — every fixable refusal stays a synchronous 4xx — then calls `execute_update_with_start_workflow`, which starts `ord::{order_id}` and awaits the `await_placement` update in ONE RPC. The workflow's first activity, `create_order`, writes the order + line snapshots (+`request_hash`) + `OrderPlaced` outbox row in one transaction and the update resolves with the order's status, which becomes the 202. Concurrent duplicates converge inside Temporal (`USE_EXISTING` attaches; both callers get the same ack), and a failure before the row exists leaves **nothing** behind — the retry just runs again onto the same id. One deliberate carve-out: a deterministic pricing refusal probes `attach_placement` first, because a retry landing in the pending window after menu drift must hear about the order being made, not "re-confirm" into a second one. Awaiting longer than `placement_await_seconds` (2s) answers 202 anyway — the workflow is durable — at the cost of a brief 404 window on the order read. *(Accepted cost: Temporal is a checkout-availability dependency — its outage is a 503 on placement. The commit-first/sweeper design and its trade-offs live in [placement-initiation-options.md](placement-initiation-options.md).)*
...
*(As built, W3: `c1.orders.events` and `c1.payments.events` have their first consumer — the notification inbox, one `EventConsumer` loop PER topic (groups `notification.inbox.orders` / `notification.inbox.payments`) — separate loops so a payments-side `ProjectionLag` backoff can never block the orders loop that arms the projection it waits for. `<topic>.dlq` topics (e.g. `c1.orders.events.dlq`) now exist — broker-auto-created, uncompacted; see the Retry/DLQ policy below and ADR-0021.)*
...
**As built: the OpenTelemetry SDK** — `TracerProvider` + `BatchSpanProcessor` + OTLP/HTTP to Jaeger, configured once per process by `setup_tracing()` (empty endpoint = off; `make up-obs` arms it — and because any other `docker compose up` silently recreates a service untraced, each process publishes a `tracing_armed` gauge and the `TracingDisarmed` alert names the drift instead of leaving it to be discovered mid-incident). SERVER spans in the shared middleware, CONSUMER spans per Kafka message, PRODUCER spans per outbox publish — parented across the async hop by the traceparent stored in the outbox column. An earlier hand-rolled OTLP exporter (git history) was replaced wholesale on review guidance: same wire format and call sites, but the SDK brings the ecosystem — httpx/SQLAlchemy auto-instrumentation and Temporal's `TracingInterceptor` are now drop-ins (both pending, below). Target state: `order_id`/`delivery_id` in W3C baggage. Outbox rows store `traceparent` as columns; a Debezium SMT (and the dev poller) lift them into Kafka headers — **the async hop stays stitched**. Temporal `TracingInterceptor` (replay-safe); `workflow_id=order_id` as a search attribute so Jaeger and Temporal Web cross-navigate on one key. Tail-based sampling keeps 100% of error/slow/payment/dispatch traces; no per-GPS-ping spans. Structured JSON logs with `trace_id` + `order_id` on every line; no PII.
