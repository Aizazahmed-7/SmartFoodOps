...
| UC-4 | Place order & pay | Customer checks out | `POST /v1/orders` (client idempotency key, per-cell admission) → OrderWorkflow: PriceOrder → ValidateAndReserve → AuthorizePayment → ConfirmOrder → NotifyRestaurant. Visible as confirmed **only after all validations** | FR-14–FR-19, FR-21–FR-22; over-capacity → 429 before any write (NFR-8); any step fails → saga compensation (FR-19) |
...
| FR-14 | Idempotent order placement | P0 | Client idempotency key; duplicate `POST /v1/orders` attaches to the running workflow (`REJECT_DUPLICATE`/`USE_EXISTING`) — never a second order |
...
| FR-16 | Immutable pricing snapshot | P0 | PriceOrder writes `{subtotal, discounts, fees, tax, total}`; authorization and refunds computed only from it; price drift at checkout returns `PRICE_CHANGED` diff for client re-confirm |
| FR-17 | Guarded, single-writer order state machine | P0 | All transitions `UPDATE … WHERE status='prev'` (0 rows = no-op); `illegal_transition_total` ≈ 0; order visible as confirmed only after validation + authorization succeed |
...
| ID | Requirement | Pri | Acceptance criteria |
|---|---|---|---|
| FR-21 | Authorize / capture / void / refund behind hexagonal `PaymentGateway` port | P0 | Authorization before confirmation; capture only after `DELIVERED`; PSP swap requires no caller changes |
...
| FR-30 | Double-assignment prevention | P0 | DDB conditional write (`attribute_not_exists(offer_lock) AND size(active_deliveries) < cap`) is the single lock authority; chaos drill: concurrent offers to same rider yield exactly one assignment |
| FR-31 | Escalation ladder | P1 | ~4 min unassigned → surge payout; ~8 min → ops manual-dispatch + customer delay notice |
| FR-32 | Failure recovery & reassignment | P0 | Liveness via disconnect >45s, heartbeat expiry 90s, 60s SCAN reconciliation; revoke is conditional (`rider_id=:expected AND state=ASSIGNED`); post-pickup failure escalates to ops, never silent |
...
| ID | Requirement | Pri | Acceptance criteria |
|---|---|---|---|
| FR-36 | Real-time customer tracking over SSE | P0 | 2s cadence en-route via Redis sharded pub/sub → tracking-gateway; 30-min hard connection lifetime |
...
| FR-50 | Governed event envelope | P0 | Avro + Schema Registry, `BACKWARD_TRANSITIVE`, CI compatibility gate; envelope: `event_id` (UUIDv7), `event_type`, `aggregate_id`, `aggregate_version`, `occurred_at`, `cell_id`; `traceparent` in headers |
