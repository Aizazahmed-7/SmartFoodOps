...
class Sweeper:
    ...
    async def sweep_once(self) -> int:
        """Start sagas for every stale PLACED order; returns how many were
        swept. Per-order failures are logged and skipped so one unreachable
        start cannot shadow the rest of the backlog."""
        ...
        for order_id in order_ids:
            try:
                ...
                continue
            # A sweep firing at all means a placement's own start call died
            # (crash or outage) — worth a loud line per healed order.
