...
def seed_menu(
    client, sub="usr_owner", name="Biryani House", cuisines=("pakistani",), tags=("halal",)
):
    ...
    item = client.post(
        ...
        json={"category_id": cat["id"], "name": "Biryani", "price_cents": 1200, "tags": list(tags)},
        headers=admin,
    ).json()
...
def test_browse_filters(client, cache):
    seed_menu(
        client, sub="usr_a", name="Biryani House", cuisines=("pakistani", "bbq"), tags=("halal",)
    )
    ...
    tagged = client.get("/v1/restaurants", params={"city": "springfield", "tag": "halal"}).json()
    assert [r["name"] for r in tagged["restaurants"]] == ["Biryani House"]

    empty_page = client.get("/v1/restaurants", params={"city": "springfield", "page": 5}).json()
...
def test_browse_tag_ignores_86d_items(client, cache):
    ...
    client.patch(f"/v1/restaurants/{rid}/items/{item_id}", json={"available": False}, headers=admin)
...
def test_browse_validation_branches(client):
    ...
    assert client.get("/v1/restaurants", params={"city": "s", "page": -1}).status_code == 422
...
def test_reads_survive_cache_down(down_client):
    ...
    assert down_client.get("/v1/restaurants", params={"city": "springfield"}).status_code == 200
